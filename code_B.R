...
### run code_A.R before running this script ###

rm(list=ls())
set.seed(123)
setwd("D:/sfu/stat 440/module2")

samples = read.table("train-2.txt", sep=' ', row.names = NULL, header = TRUE)
co = cor(sample[,is.numeric(samples)], method = "spearman")
corrplot::corrplot(co, type = "upper")
...
tc = makeTuneControlRandom(maxit = 200)
...
r = resample(gbm_lrn, tsk, resampling = rdesc, show.info = T, models = FALSE,measures = mae)
#------------------------------------
